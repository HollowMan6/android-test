...
public class InstrumentationCoverageReporter {
  ...
  private String dumpCoverageToFile(
    ...
    if (coverageFilePath == null) {
      Log.d(TAG, "No coverage file path was specified. Dumps to the default path.");
      coverageFilePath =
  ...
  private void moveFileToTestStorage(String srcFilePath, String destFilePath) throws IOException {
    ...
    if (srcFile.exists()) {
      Log.d(
          TAG,
          String.format(
              "Copying coverage file [%s] to the internal test storage [%s].",
  ...
  /** Returns whether the test storage service is available. */
